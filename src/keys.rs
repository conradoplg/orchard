...
impl FullViewingKey {
    /// Create a `FullViewingKey` intended *ONLY FOR EXPERIMENTAL USE* with
    /// FROST, and that *DOES NOT SUPPORT QUANTUM RECOVERABILITY*, from
    /// the given `SpendingKey` `sk` and the given `SpendValidatingKey` `ak`.
    ///
    ...
    /// not derived from a `SpendingKey` (since it might be generated using a
    /// Distributed Key Generation protocol, which generates a random
    /// `SpendValidatingKey`). This method achieves this by creating the
    /// `FullViewingKey` from the `SpendingKey` as usual, with the exception of
    /// `SpendValidatingKey` which is provided by the caller.
    ///
    /// # Why this method of constructing FVKs will not be supported in the long term
    ///
    /// See [ZIP 2005](https://zips.z.cash/zip-2005) for the motivation behind
    /// Quantum Recoverability. A Quantum Recovery protocol needs to be able to
    /// check that $\mathsf{ivk}$ commits to the full viewing key $(\mathsf{ak}, \mathsf{nk})$.
    /// Because the commitment scheme used in Orchard to derive the FVK is not
    /// post-quantum binding, the Recovery circuit has to check (in zk) how the ivk
    /// was constructed, which requires a known key derivation to be used. This
    /// constructor bypasses the normal key derivation for non-FROST keys, and
    /// the key derivation for FROST keys is yet to be fully specified (the choice of
    /// $\mathsf{H^{qk}}$ has not been made).
    ///
    /// The impact of this is not limited to funds sent to the corresponding address
    /// being quantum-vulnerable. Because FVKs generated by this constructor will
    /// not conform to future mandatory conformance requirements, future wallets
    /// might not support them at all, possibly resulting in the funds becoming
    /// inaccessible. That is why this constructor is only intended for experimental
    /// usage, and SHOULD NOT be exposed in public-facing wallet software without
    /// strong warnings to the user.
    #[cfg(feature = "unstable-frost")]
    pub fn from_sk_ak_incompatible_with_quantum_recoverability_and_will_be_removed(
        sk: &SpendingKey,
        ak: SpendValidatingKey,
    ) -> Self {
        FullViewingKey {
